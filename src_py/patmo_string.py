...
def fileReplaceBuild(sourceFile, destinationFile, pragmaList=[], replaceList=[], pragmaIfList=[],
                     ifList=[]):
    # check for errors
    if len(pragmaList) != len(replaceList):
        sys.exit("ERROR: pragma list lenght is different from replace list!")
    if len(pragmaIfList) != len(ifList):
        sys.exit("ERROR: pragma IF list lenght is different from replace list!")
    if not os.path.isfile(sourceFile):
        sys.exit("ERROR: source file " + sourceFile + " not found!")
    if ("src_f90" in destinationFile) or ("src_py" in destinationFile):
        sys.exit("ERROR: you are writing in src_* folders!")

    # sort pragma by length in order to have the longer first (to avoid misreplacing)
    pragamaIndexes = [i[0] for i in
                      sorted(enumerate(pragmaList), key=lambda x: len(x[1]), reverse=True)]
    pragamaIfIndexes = [i[0] for i in
                        sorted(enumerate(pragmaIfList), key=lambda x: len(x[1]), reverse=True)]
    ...
    for row in fh:
        ...
        # add replaced line to full file string
        full_file += row
...
def indentF90(filename):
    ...
    tokenclose = ["end do", "end if", "end function", "end subroutine", "else if", "elseif", "else",
                  "enddo", "end module",
                  "endif", "end type", "endtype", "contains", "endfunction", "endsubroutine",
                  "endmodule", "end program", "endprogram",
    ...
    for row in fh:
        ...
        if lbeg(tokenclose, srow):
            nind -= 1  # check if the line ends with one of tokenclose
        indent = (" " * (nind * nspace))  # compute number of spaces for indent
        if is_amper:
            indent = (" " * (2 * nspace)) + indent  # increas indent in case of previous &
        if srow.startswith("#"):
            indent = ""  # no indent for pragmas
        if not(srow == "" and is_blank):
            arow.append(indent + srow + "\n")  # append indented line to array of rows
        is_amper = False  # is a line after ampersend flag
        if lend(["&"], srow):
            is_amper = True  # check if the line ends with &
        is_blank = (srow == "")  # flag for blank line mode
        if lbeg(tokenopen, srow):
            nind += 1  # check if the line ends with one of tokenclose
        if lbeg(["if"], srow) and "then" in srow:
            nind += 1  # check if line stats with if and has then
        if srow == "do":
            nind += 1
    fh.close()

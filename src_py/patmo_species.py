...
class species:
    ...
    fh = open("data/parseReference/reference.dat", "rb")
    for row in fh:
    ...
    def getHtmlName(self):
        ...
        for char in self.name[:]:
            ...
            if patmo_string.isInteger(char) and charOld not in noSubChar:
                char = "<sub>" + char + "</sub>"
            self.htmlName += char
            charOld = char
        return self.htmlName
    ...
    def parse(self, spec):
        ...
        for i in range(len(parts)):
            ...
            if part.strip() == "":
                continue
            # skip numbers
            if patmo_utils.isNumber(parts[i]):
                continue
            ...
            if i < len(parts) - 1:
                ...
                if patmo_utils.isNumber(parts[i + 1]):
                    multi = int(parts[i + 1])

...
class reaction:
    ...
    def makeHtmlPage(self):
        ...
        fout.write(
            "<img src=\"ratePNGs/rate" + str(int(1e6) + self.index) + ".png\" width=\"500px\">\n")
        fout.write("<img src=\"rateReversePNGs/rate" + str(
            int(1e6) + self.index) + ".png\" width=\"500px\">\n")
        href_bak = "rate" + str(int(1e6) + self.index - 1) + ".html"
        href_fwd = "rate" + str(int(1e6) + self.index + 1) + ".html"
        fout.write("<br><br><a href=\""+href_bak+"\">&#8882; previous rate</a>")
        fout.write("&nbsp;&nbsp;|&nbsp;&nbsp;")
        fout.write("<a href=\""+href_fwd+"\">next rate &#8883;</a>")
    ...
    def isContainingSpeciesName(self, speciesName):
        if self.isContainingSpeciesNameReactants(speciesName):
            return True
        if self.isContainingSpeciesNameProducts(speciesName):
            return True
    ...
    def getHtmlTableRow(self, highlightSpecies=None):
        ...
        if self.photoIndex is not None:
            rowReactants[1] = "<i>h&nu;</i>"
        # put the pieces together
        rowString = "<tr><td>" + str(self.index) + "<td>" + (
            "<td>+<td>".join(rowReactants)) + "<td>&#8594;<td>" \
    ...
    def getHtmlVerbatim(self):
        if self.htmlVerbatim is not None:
    ...
    def getHashReverse(self):
        if self.hashReverse is not None:
    ...
    def getFileSafeVerbatim(self):
        if self.fileSafeVerbatim is not None:
    ...
    def getReverse(self, thermochemistry):
        ...
        for species in reverse.reactants:
            ...
            if Tmid == 0e0:
                Tmid = thermochemistry[species.name]["Tmid"]

            if Tmin == 0e0:
                Tmin = thermochemistry[species.name]["Tmin"]
        ...
        polyTmult = ["*(lnTgas(i)-1d0)", "*Tgas(i)", "*Tgas2(i)", "*Tgas3(i)", "*Tgas4(i)",
                     "*invTgas(i)", ""]
        polyLow = []
        polyUp = []
        ...
        # join polyomials, replace +- signs, add line returns for readability
        polyLowJoint = (" + ".join(polyLow)).replace(" + -", " - ")\
            .replace(" + ", " &\n+ ").replace(" - ", " &\n- ")
        polyUpJoint = (" + ".join(polyUp)).replace(" + -", " - ").replace(" + ", " &\n+ ").replace(
            " - ", " &\n- ")

        diffFactor = ""
        # note here is (kb*T/P0)**(R-P) where P and R are referred to forward
        # so for the reverse is the opposite (R-P)
        # P0 is the standard pressure at 1 bar=1e6 barye
    ...
    # *******************
    # TODO: plot photo rates
    def plotPhotorate(self):
        rate = 0e0
        planck_eV = 4.135667516e-15  # eV*s
        # flux = getBB()
        for ienergy in range(len(self.xsecEnergySpan)):
            rate += flux[ienergy] * self.xsecEnergySpan[ienergy] * self.xsec[ienergy] / \
                    self.xsecEnergy[ienergy] / planck_eV
            fout.write(str(self.xsecEnergy[ienergy]) + " " + str(self.xsec[ienergy]) + "\n")

    # ******************
    # dump the cross section to a fileName contained in folder, with specific postpone string
    ...
    def parse(self, rowList, formatList):
        ...
        if len(rowList) is not len(formatList):
            print formatList
            print rowList
        ...
        for i in range(len(rowList)):
            ...
            # rate in F90 format
                ...
                if "ntot(icell)" in self.rate:
                    self.isPseudo3body = True
            else:
    ...
    def parseKIDA(self, reactants, products, data):
        ...
        if formulaType == 1:
            ...
            if kb != 0:
                krate += "*(Tgas/3d2)**(" + str(float(kb)) + ")"
            if kc != 0:
                krate += "*exp(-" + patmo_string.f90DoubleCompact(kc) + "/Tgas)"
    ...
    def parseKIDA3b(self, reactants, products, data):
        ...
        if data["kb_low"] != 0:
            klow += "*(Tgas/3d2)**(" + kb_low + ")"
        if data["kc_low"] != 0:
            klow += "*exp(-" + kc_low + "/Tgas)"
        kinf = "kinf(icell) = " + ka_inf
        if data["kb_inf"] != 0:
            kinf += "*(Tgas/3d2)**(" + kb_inf + ")"
        if data["kc_inf"] != 0:
            kinf += "*exp(-" + kc_inf + "/Tgas)"
        ...
        if data["Pmin"] == -9999 and data["Pmax"] == 9999:
            ...
            limitsPressureBegin = "if(Pr(icell).le." + patmo_string.f90DoubleCompact(
                data["Pmax"]) + ") then\n"
            limitsPressureEnd = "end if"
        elif data["Pmin"] != -9999 and data["Pmax"] == 9999:
            limitsPressureBegin = "if(Pr(icell).ge." + patmo_string.f90DoubleCompact(
                data["Pmin"]) + ") then\n"
            limitsPressureEnd = "end if"
        else:
            limitsPressureBegin = "if(Pr(icell).ge." + patmo_string.f90DoubleCompact(data["Pmin"]) \
                                  + " .and. Pr(icell).le." + patmo_string.f90DoubleCompact(
                data["Pmax"]) + ") then\n"
            limitsPressureEnd = "end if"

...
def buildUtils(network):
    ...
    for atomName in network.getAtoms():
        ...
        for species in network.getSpecies():
            if atomName not in species.atomDict:
                continue
            mult = ""
            if species.atomDict[atomName] > 1:
                mult = " * " + patmo_string.f90DoubleCompact(
                species.atomDict[atomName])
            allMassFunctions += functionName + " = " + functionName + " + m(" + species.label \
                                + ") * nall(icell," + species.label + ")" + mult + "\n"
        allMassFunctions += "  end do\n\n"
        allMassFunctions += "end function\n\n"

    pragmaList = ["#PATMO_mass", "#PATMO_speciesNames", "#PATMO_flux", "#PATMO_weightedDegree",

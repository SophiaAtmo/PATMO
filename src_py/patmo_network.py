...
class network:
    ...
    def loadNetwork(self):
        ...
        if fname == "":
            ...
            if not self.options.species:
                print "ERROR: in option file"
                sys.exit()
            return

        aformat = None  # trigs error if missing format in network file
    ...
    def loadKIDA3body(self, speciesReferenceList):
        ...
        for row in fh:
            ...
            for species in (reactants + products):
                ...
                if species not in speciesReferenceList:
    ...
    def loadKIDA2body(self, speciesReferenceList):
        ...
        for row in fh:
            ...
            for i in range(len(arow)):
                ...
                if patmo_string.isNumber(arow[i]):
                    break
            ...
            if myHash in reactionDictionary:
                ...
                if contiguousA or contiguousB:
                    rateIf = None  # to trig error
                    if contiguousA:
                        rateIf = "if(Tgas<" + patmo_string.f90DoubleCompact(myReaction.TgasMax) \
                                 + ") then\n"
                    if contiguousB:
                        rateIf = "if(Tgas>" + patmo_string.f90DoubleCompact(myReaction.TgasMin) \
                                 + ") then\n"
        ...
        for (rhash, myReaction) in reactionDictionary.iteritems():
            myReaction.index = len(self.reactions) + 1
            self.reactions.append(myReaction)
    ...
    def getSinkSourceList(self):
        ...
        for reactant in allReactants:
            ...
            if reactant not in allProducts:
                sList["source"].append(reactant)
        # loop on products
        for product in allProducts:
            # if product not present in reactants store it
            if product not in allReactants: sList["sink"].append(product)
        return sList
    ...
    def loadThermochemistry(self):
        ...
        # trigs error in case of wrong read
        upCoef = lowCoef = data = speciesName = None

        # open file to read
        fh = open(self.fileThermochemistry, "rb")
        for row in fh:
            srow = row.strip()
            if srow == "":
                continue
    ...
    def getMaxReactants(self):
        if self.maxReactants is not None:
    ...
    def getMaxProducts(self):
        if self.maxProducts is not None:
    ...
    def getSpecies(self):
        ...
        self.species = []
        for name, species in specDict.iteritems():
    ...
    def getSpeciesNames(self):
        if self.speciesNames is not None:
    ...
    def reverseCheck(self):
        ...
        for rdata in reactionsData:
            ...
            if (rdata["hashRev"] not in hashReference) or (not self.options.useEntropyProduction):
    ...
    # *********************
    # prepare html docs
    def makeHtmlDocs(self, options):
        shutil.copyfile("htmlDocs/index.src", "htmlDocs/index.html")
        self.makeHtmlSpeciesList()
        self.makeHtmlReactionsList()
        if options.doTopology:
            self.createTopology()
        if options.plotThermochemistry:
            self.plot_thermochemistry_all()
        self.plotRates()
    ...
    def makeHtmlSpeciesList(self):
        ...
        for species in speciesList:
            # write table entry
            fout.write(
                "<tr><td>" + species.getHtmlName() + "<td><a href=\"species_"
                + species.name + ".html\">details</a><br>\n")
    ...
    def plotRates(self):
        ...
        aTgas = [1e1 ** (i * (logTmax - logTmin) / (npoints - 1)
                         + logTmin) for i in range(npoints)]
        ...
        for reaction in self.reverseReactions:
            ...
            for Tgas in aTgas:
                if Tgas < reaction.Tmid:
                    rate = "(" + refRates[reaction.reverseOriginalIndex] \
                           + ")*" + reaction.rateLow
                else:
                    rate = "(" + refRates[reaction.reverseOriginalIndex] \
                           + ")*" + reaction.rateUp
    ...
    # **********************
    def plot_thermochemistry_all(self):
        for what in ["G", "S", "Cp", "H"]:
            self.plot_thermochemistry(what)

    # **********************
    def plot_thermochemistry(self, what="gibbs"):
        import numpy as np

        # ouput folder
        outFolder = "htmlDocs/thermochemistryPNGs/"
        # create output folder if not present
        if not os.path.exists(outFolder):
            os.makedirs(outFolder)

        poly_mult = poly_fact = ylab = what_file = None
        if what in ["gibbs", "G"]:
            poly_mult = ["*(1e0-np.log(x))", "*x", "*x**2", "*x**3", "*x**4",
                         "/x", ""]
            poly_fact = [1e0, -1e0 / 2e0, -1e0 / 6e0, -1e0 / 12e0, -1e0 / 20e0, +1e0, -1e0]
            ylab = "G/RT"
            what_file = "gibbs"
        elif what in ["entropy", "S"]:
            # entropy
            poly_mult = ["*np.log(x)", "*x", "*x**2", "*x**3", "*x**4", "", ""]
            poly_fact = [1e0, 1e0, 1e0 / 2e0, 1e0 / 3e0, 1e0 / 4e0, 0e0, 1e0]
            ylab = "S/R"
            what_file = "entropy"
        elif what in ["Cp"]:
            # Cp
            poly_mult = ["", "*x", "*x**2", "*x**3", "*x**4"]
            poly_fact = [1e0] * len(poly_mult)
            ylab = "Cp/R"
            what_file = "heatcap"
        elif what in ["enthalpy", "H"]:
            # H
            poly_mult = ["", "*x", "*x**2", "*x**3", "*x**4", "1/x"]
            poly_fact = [1e0, 1e0/2e0, 1e0/3e0, 1e0/4e0, 1e0/5e0, 1e0]
            what_file = "enthalpy"
            ylab = "H/RT"
        else:
            print "ERROR: cannot plot thermochemistry of " + what
            patmo_error.trigError(__file__, inspect.currentframe())

        if len(poly_mult) != len(poly_fact):
            print "ERROR: poly mult and poly fact have different size in plot thermochemistry!"
            patmo_error.trigError(__file__, inspect.currentframe())

        for species_name, data in self.thermochemistry.iteritems():
            plt.clf()
            print "plot thermochemistry (" + what + ") " + species_name
            params = [[data["Tmin"], data["Tmid"], data["lowCoef"]],
                      [data["Tmid"], data["Tmax"], data["upCoef"]]]
            for Tstart, Tend, coeff in params:
                xdata = np.logspace(np.log10(Tstart), np.log10(Tend))
                expr = []
                for ii in range(len(poly_mult)):
                    expr += ["+(" + str(coeff[ii] * poly_fact[ii]) + poly_mult[ii] + ")"]
                poly = " ".join(expr)
                ydata = [eval(poly) for x in xdata]
                plt.plot(xdata, ydata)
            plt.title(species_name)
            plt.xlabel("T/K")
            plt.ylabel(ylab)
            fnamePNG = outFolder + "poly_" + what_file + "_" + species_name + ".png"
            plt.savefig(fnamePNG)
    ...
    def buildODE(self):
        ...
        for reaction in (self.reactions + self.photoReactions + self.reverseReactions):
            ...
            for product in reaction.products:
                ...
                ODEdict[key].append(" + " + reaction.getRHS())
        fullODE = ""
        for (ode, RHS) in ODEdict.iteritems():
            fullODE += ode + " = &\n" + (" &\n".join(RHS)) + "\n\n"
    ...
    def createTopology(self):
        ...
        for edge in connections:
            fout.write(str(icount + 1) + ", " + (", ".join([str(idxMap["\"" + x + "\""])
                                                            for x in edge])) + "\n")
            icount += 1
        fout.close()
